...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const valA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const valB = Math.sqrt(x2 ** 2 + y2 ** 2);
  if (valA === 0 || valB === 0) {
    throw new Error('val != 0');
  }
  const cosCorner = dot / (valA * valB);
  const clampedCosCorner = Math.min(1, Math.max(-1, cosCorner));
  return Math.acos(clampedCosCorner);
}
...
function getSumOfDigits(num) {
  ...
  const sum = arr.reduce((acc, digit) => acc + Number(digit), 0);
  return sum;
}
...
function getCountOfOddNumbers(number) {
  ...
  const plusNumber = Math.abs(number);
  for (let i = 1; i <= plusNumber; i += 1) {
    if (i % 2 !== 0) {
      result += 1;
    }
  }
